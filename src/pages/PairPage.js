...
function PairPage({ pairAddress, history }) {
  ...
  // token data for usd
  const token0USD = token0?.derivedUSD ? formattedNum(parseFloat(token0.derivedUSD), true) : ''
  const token1USD = token1?.derivedUSD ? formattedNum(parseFloat(token1.derivedUSD), true) : ''
            ...
            >
              <FixedPanel onClick={() => history.push(`/token/${token0?.id}`)}>
                <RowFixed>
                  ...
                  <TYPE.main fontSize={'16px'} lineHeight={1} fontWeight={500} ml={'4px'}>
                    {token0 && token1
                      ? `1 ${formattedSymbol0} = ${token0Rate} ${formattedSymbol1} ${token0USD ? '(' + token0USD + ')' : ''}`
                      : '-'}
                  </TYPE.main>
                </RowFixed>
              </FixedPanel>
              <FixedPanel onClick={() => history.push(`/token/${token1?.id}`)}>
                <RowFixed>
                  ...
                  <TYPE.main fontSize={'16px'} lineHeight={1} fontWeight={500} ml={'4px'}>
                    {token0 && token1
                      ? `1 ${formattedSymbol1} = ${token1Rate} ${formattedSymbol0}  ${token1USD ? '(' + token1USD + ')' : ''}`
                      : '-'}
                  </TYPE.main>
                </RowFixed>
              </FixedPanel>
